# @see https://firebase.google.com/docs/auth/admin/verify-id-tokens?hl=ja#verify_id_tokens_using_a_third-party_jwt_library
#
# Usage:
...
module FirebaseAuthenticator
  ...
  PROJECT_ID = Rails.application.credentials[:fb_project_id]
  ...
  def decode!(token)
    options = {
      algorithm: ALG,
      iss: ISSUER_URI_BASE + PROJECT_ID, # ID Tokenの発行元URIと一致することを確認
      verify_iss: true,
      aud: PROJECT_ID,
      verify_aud: true, # "client_id"の値と一致することを確認
      verify_iat: true, # 発行時間が現在時刻よりもはるか昔でないことを確認
    }
